...
public class VideoGridCardBinding extends android.databinding.ViewDataBinding  {
    ...
    public final android.widget.RelativeLayout videoGridItemLayout;
    public final android.widget.LinearLayout videoMeta;
    // variables
    ...
    public VideoGridCardBinding(android.databinding.DataBindingComponent bindingComponent, View root) {
        ...
        this.itemMore = (android.widget.ImageView) bindings[7];
        this.itemMore.setTag(null);
        this.mboundView2 = (android.widget.LinearLayout) bindings[2];
        this.mboundView2.setTag(null);
        this.mlItemProgress = (android.widget.ProgressBar) bindings[8];
        ...
        this.videoGridItemLayout = (android.widget.RelativeLayout) bindings[0];
        this.videoMeta = (android.widget.LinearLayout) bindings[4];
        this.videoMeta.setTag(null);
        setRootTag(root);
        // listeners
        invalidateAll();
    }
    ...
    protected void executeBindings() {
        ...
        boolean textUtilsIsEmptyTimeTextUtilsIsEmptyResolutionBooleanFalse = false;
        ...
        int textUtilsIsEmptyTimeTextUtilsIsEmptyResolutionBooleanFalseViewGONEViewVISIBLE = 0;
        ...
        if ((dirtyFlags & 0x109L) != 0) {
                ...
                // read TextUtils.isEmpty(time) ? TextUtils.isEmpty(resolution) : false
                textUtilsIsEmptyTimeTextUtilsIsEmptyResolutionBooleanFalse = ((textUtilsIsEmptyTime) ? (textUtilsIsEmptyResolution) : (false));
            if((dirtyFlags & 0x109L) != 0) {
                if(textUtilsIsEmptyTimeTextUtilsIsEmptyResolutionBooleanFalse) {
                        dirtyFlags |= 0x1000L;
                }
                else {
                        dirtyFlags |= 0x800L;
                ...
                // read TextUtils.isEmpty(time) ? TextUtils.isEmpty(resolution) : false ? View.GONE : View.VISIBLE
                textUtilsIsEmptyTimeTextUtilsIsEmptyResolutionBooleanFalseViewGONEViewVISIBLE = ((textUtilsIsEmptyTimeTextUtilsIsEmptyResolutionBooleanFalse) ? (android.view.View.GONE) : (android.view.View.VISIBLE));
        }
        // batch finished
        if ((dirtyFlags & 0x180L) != 0) {
            // api target 1
            ...
            this.videoGridItemLayout.setOnClickListener(holderOnClickAndroidViewViewOnClickListener);
            this.videoGridItemLayout.setOnLongClickListener(holderOnLongClickAndroidViewViewOnLongClickListener);
        }
        if ((dirtyFlags & 0x109L) != 0) {
            // api target 1

            org.videolan.vlc.gui.video.VideoListAdapter.setLayoutHeight(this.mboundView2, time, resolution);
            this.videoMeta.setVisibility(textUtilsIsEmptyTimeTextUtilsIsEmptyResolutionBooleanFalseViewGONEViewVISIBLE);
        }
        if ((dirtyFlags & 0x120L) != 0) {
            // api target 1
    ...
    /* flag mapping
        ...
        flag 7 (0x8L): holder
        flag 8 (0x9L): null
        flag 9 (0xaL): TextUtils.isEmpty(time) ? TextUtils.isEmpty(resolution) : false
        flag 10 (0xbL): TextUtils.isEmpty(time) ? TextUtils.isEmpty(resolution) : false
        flag 11 (0xcL): TextUtils.isEmpty(time) ? TextUtils.isEmpty(resolution) : false ? View.GONE : View.VISIBLE
        flag 12 (0xdL): TextUtils.isEmpty(time) ? TextUtils.isEmpty(resolution) : false ? View.GONE : View.VISIBLE
        flag 13 (0xeL): progress == 0 ? View.INVISIBLE : View.VISIBLE
        flag 14 (0xfL): progress == 0 ? View.INVISIBLE : View.VISIBLE
    flag mapping end*/
    //end
}